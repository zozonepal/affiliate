...
export function Hero({
  ...
  return (
    <section className="bg-gradient-to-b from-orange-600 via-orange-500 to-amber-500 text-white py-3 px-3 sm:pt-6 sm:pb-7 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto text-center relative z-10">
        ...
        <div className="hidden sm:inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/20 backdrop-blur-md text-orange-50 text-xs font-semibold mb-2 border border-white/25 shadow-2xs">
          <Tag className="w-3.5 h-3.5 text-amber-200" />
          <span>Curated Daily for Nepal 🇳🇵</span>
        </div>

        {/* Main Title */}
        <h1 className="text-base sm:text-3xl font-black tracking-tight text-white mb-1 sm:mb-1.5 leading-tight">
          Best Budget Tech & Lifestyle Deals
        </h1>
        <p className="hidden sm:block text-orange-100 text-xs sm:text-sm max-w-xl mx-auto mb-3.5 font-normal leading-relaxed px-2">
          Handpicked top-rated gadgets and lifestyle picks verified for authentic discounts on Daraz Nepal.
        </p>

        {/* Search & Filter Bar Container */}
        <div className="bg-white p-1.5 sm:p-2.5 rounded-xl sm:rounded-2xl shadow-md sm:shadow-lg max-w-3xl mx-auto border border-orange-200/40">
          <div className="flex gap-1.5 sm:gap-2 items-center">
            ...
            <div className="relative flex-1 min-w-0">
              ...
              <input
                ...
                className="w-full pl-8 sm:pl-10 pr-7 sm:pr-9 py-1.5 sm:py-2.5 rounded-lg sm:rounded-xl text-slate-900 placeholder-slate-400 text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 bg-slate-50 hover:bg-white transition min-h-[38px] sm:min-h-[44px]"
              />
              {searchQuery && (
                <button
            ...
            <div className="relative w-28 sm:w-44 shrink-0">
              <ArrowUpDown className="absolute left-2 sm:left-3.5 top-1/2 -translate-y-1/2 w-3 h-3 sm:w-3.5 sm:h-3.5 text-slate-500 pointer-events-none" />
              <select
                id="sort-select"
                value={sortOption}
                onChange={(e) => onSortChange(e.target.value as SortOption)}
                className="w-full pl-6 sm:pl-9 pr-2 sm:pr-4 py-1.5 sm:py-2.5 bg-slate-50 hover:bg-white text-slate-700 text-[11px] sm:text-xs font-semibold rounded-lg sm:rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-orange-500 cursor-pointer appearance-none min-h-[38px] sm:min-h-[44px] truncate"
              >
          ...
          <div className="flex items-center gap-1 sm:gap-1.5 overflow-x-auto pt-1.5 sm:pt-2 mt-1.5 sm:mt-2 border-t border-slate-100 scrollbar-none -mx-1 px-1 touch-pan-x">
            <span className="text-[10px] sm:text-[11px] font-bold text-slate-400 uppercase tracking-wider pl-1 hidden sm:inline">
              Price:
            </span>
            {pricePills.map((pill) => (
              <button
                ...
                className={`px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-lg text-[11px] sm:text-xs font-medium whitespace-nowrap min-h-[30px] sm:min-h-[36px] flex items-center justify-center transition active:scale-95 shrink-0 ${
                  priceFilter === pill.id
